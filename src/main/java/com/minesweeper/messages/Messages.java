package com.minesweeper.messages;

public class Messages {

    public static final String WELCOME_MESSAGE = "Welcome to Minesweeper!";
    public static final String GRID_MESSAGE = "Enter the size of the grid (e.g. 4 for a 4x4 grid): ";
    public static final String MINE_MESSAGE = "Enter the number of mines to place on the grid (maximum is 35% of the total squares): ";
    public static final String MINEFIELD_MESSAGE = "Here is your minefield:";
    public static final String SELECT_SQUARE_MESSAGE = "Select a square to reveal (e.g. A1): ";
    public static final String UPDATED_MINEFIELD_MESSAGE = "Here is your updated minefield:";
    public static final String INCORRECT_MESSAGE = "Incorrect input.";
    public static final String DETONATED_MINE_MESSAGE = "Oh no, you detonated a mine! Game over.";
    public static final String WON_MESSAGE = "Congratulations, you have won the game!";
    public static final String PLAY_AGAIN_MESSAGE = "Press any key to play again...";
    public static final String SQUARE_INFO_FORMAT = "This square contains {0} adjacent mines.";
    public static final String MIN_GRID_MESSAGE = "Minimum size of grid is ";
    public static final String MAX_GRID_MESSAGE = "Maximum size of grid is ";
    public static final String MAX_MINE_COUNT_MESSAGE = "Maximum number is 35% of total squares.";
    public static final String AT_LEAST_ONE_MINE_MESSAGE = "There must be at least 1 mine.";

}
